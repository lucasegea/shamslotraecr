...
class UncategorizedScraper:
    ...
    def process_product_page(self, url):
        ...
        try:
            ...
            # Método 1: Buscar en h1 o h2 principal
            ...
            # Método 2: Buscar en selectores específicos si no se encontró en h1/h2
            if not name:
            ...
            # Método 3: Buscar por texto en negrita que podría ser un título
            if not name:
            ...
            # Método 4: Buscar texto cercano a "Título" o "Producto"
            if not name:
            ...
            if not name:
                # Buscar el texto más largo en la página que podría ser un título
                paragraphs = soup.find_all(['p', 'div', 'span'])
                longest_text = ""
                for p in paragraphs:
                    text = p.text.strip()
                    if len(text) > len(longest_text) and len(text) < 150 and text.lower() != "ofertasb":
                        longest_text = text
                
                if len(longest_text) > 15:
                    name = longest_text
                    print(f"Nombre asignado usando texto más largo: {name}")
                else:
            ...
            if image_url:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if len(image_extension) > 4:  # Si la extensión es inválida
                            image_extension = 'jpg'
                        ...
                        if isinstance(result, dict) and result.get('Key'):
                            ...
                            image_stored_url = self.supabase.storage.from_(IMAGE_BUCKET).get_public_url(storage_path)
                            print(f"✅ Imagen guardada en Supabase: {storage_path}")
                            self.stats["images_saved"] += 1
                        else:
                            print(f"⚠️ Error guardando imagen en Supabase: {result}")
